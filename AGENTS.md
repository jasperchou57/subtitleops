...
**SubtitleOps** — a browser-based subtitle toolkit migrating to TanStack Start
+ React 19 on Cloudflare Workers. The licensed `mkfast-template` foundation
provides Better Auth, Stripe / Creem, R2, D1, and an admin dashboard. Auth and
payment are intentionally disabled until production bindings and plans exist.
...
### Request Flow
Incoming request → Cloudflare Worker (`src/server.ts`) → TanStack Start handler
→ React SSR → client-side hydration. Subtitle conversion stays in the browser;
SaaS server functions are activated only after production configuration.
...
| `src/subtitleops/pages/` | Migrated public pages and tool content |
