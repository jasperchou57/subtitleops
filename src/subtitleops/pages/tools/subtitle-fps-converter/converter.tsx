...
export function SubtitleFpsConverter() {
  ...
  const resolvedSource =
    sourceFps === CUSTOM ? parseFloat(customSource) : parseFloat(sourceFps);
  const resolvedTarget =
    targetFps === CUSTOM ? parseFloat(customTarget) : parseFloat(targetFps);
  const ratio = useMemo(() => {
    if (
      !Number.isFinite(resolvedSource) ||
  ...
  const handleConvert = useCallback(() => {
    ...
    if (
      ...
      trackEvent({
        tool: TOOL_ID,
        action: 'convert_error',
        error_type: 'invalid_fps',
        trace_id: traceId,
      });
      setError({
        message: 'Source and target FPS must be positive numbers.',
        traceId,
      });
      return;
    }
    if (resolvedSource === resolvedTarget) {
      ...
      setError({
        message:
          'Source and target FPS are the same — nothing to rescale. Pick different values.',
        traceId,
      });
      return;
    }
    ...
    try {
      ...
      const errorType =
        err instanceof Error ? err.message : 'fps_conversion_failed';
      logTrace({
        traceId,
        tool: TOOL_ID,
        action: 'error',
      ...
      setError({
        message: 'FPS conversion failed. Please check the file and FPS values.',
        traceId,
      });
    }
  ...
  return (
    <div
      data-analytics-area="fps_converter"
      ...
      {!file ? (
        ...
        <div className="rounded-2xl border bg-card p-6 md:p-8 animate-in fade-in duration-300">
          ...
          <p className="mb-6 text-xs text-muted-foreground">
            Rescales every timestamp by{' '}
            <span className="font-mono">source / target</span>
            {ratio !== null && (
              <>
                {' '}
                — current ratio{' '}
                <span className="font-mono">{ratio.toFixed(5)}</span>
                {ratio < 1
                  ? ' (faster playback, cues pulled earlier)'
                  : ' (slower playback, cues pushed later)'}
              </>
            )}
            .
          </p>
              ...
              <div className="mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <p className="font-medium text-green-700">
                    Rescaled {result.cueCount}{' '}
                    {result.cueCount === 1 ? 'cue' : 'cues'} by ratio{' '}
                ...
                {result.clampedCount > 0 && (
                  <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50/80 p-3 text-sm text-amber-800">
                    ...
                    {result.clampedCount === 1 ? '' : 's'} rounded below zero.
                    If your first cues look wrong, check that you have the
                    source and target the right way around.
                  </div>
                )}
