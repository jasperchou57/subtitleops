...
export function SubtitleShiftConverter() {
  ...
  const handleFile = useCallback(async (f: File) => {
    ...
    if (!detected) {
      ...
      setError({
        message: 'Only SRT and VTT files are supported by this tool.',
        traceId,
      });
      return;
    }
  ...
  const handleShift = useCallback(() => {
    ...
    if (!Number.isFinite(seconds)) {
      ...
      trackEvent({
        tool: TOOL_ID,
        action: 'convert_error',
        error_type: 'invalid_shift',
        trace_id: traceId,
      });
      setError({
        message: 'Enter a valid number of seconds (e.g. 2.5 or -1.25).',
        traceId,
      });
      return;
    }
    if (seconds === 0) {
      ...
      trackEvent({
        tool: TOOL_ID,
        action: 'convert_error',
        error_type: 'zero_shift',
        trace_id: traceId,
      });
      setError({
        message: 'Shift is zero — nothing to change. Enter a non-zero value.',
        traceId,
      });
      return;
    }
    ...
    try {
      ...
      const { output, cueCount, clampedCount } = shiftSubtitles(
        file.content,
        format,
        shiftMs
      );
      ...
      if (cueCount === 0) {
        ...
        setError({
          message: 'No subtitle timing lines were found in this file.',
          traceId,
        });
        return;
      }
      ...
      setError({
        message:
          'Timing shift failed. Please check that the file is a valid SRT or VTT subtitle file.',
        traceId,
      });
    }
  ...
  const handleDownload = useCallback(() => {
    ...
    const absLabel = Math.abs(seconds)
      .toFixed(3)
      .replace(/\.?0+$/, '')
      .replace('.', 'p');
    a.download =
      file.name.replace(/\.[^.]+$/, '') +
      `.shifted-${sign}${absLabel}s.${format}`;
    a.click();
  ...
  return (
    <div
      data-analytics-area="timing_shift_converter"
      data-analytics-tool={TOOL_ID}
      ...
      {!file ? (
        ...
        >
          <input
            ...
            type="file"
            accept=".srt,.vtt"
            aria-label="Upload an SRT or VTT subtitle file to shift timing"
            onChange={(e) => {
        ...
        <div className="rounded-2xl border bg-card p-6 md:p-8 animate-in fade-in duration-300">
          <div className="flex items-center gap-3 mb-6">
            ...
            <button
              type="button"
              data-analytics-control="change_file"
              onClick={handleReset}
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={() =>
                  setShiftInput((v) =>
                    (
                      Math.round(((parseFloat(v) || 0) - 0.5) * 1000) / 1000
                    ).toString()
                  )
                }
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Positive values delay subtitles (shift later). Negative values
              pull them earlier. Supports decimals, e.g.{' '}
              <span className="font-mono">2.5</span> or{' '}
              ...
              <div className="mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <p className="font-medium text-green-700">
                    Shifted {result.cueCount}{' '}
                    {result.cueCount === 1 ? 'cue' : 'cues'} by{' '}
                    {parseFloat(shiftInput)}s
                  </p>
                </div>
                ...
                {result.clampedCount > 0 && (
                  <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50/80 p-3 text-sm text-amber-800">
                    ...
                    {result.clampedCount} timestamp
                    {result.clampedCount === 1 ? '' : 's'} went below zero and{' '}
                    {result.clampedCount === 1 ? 'was' : 'were'} clamped to
                    00:00:00. Your negative shift is larger than the earliest
                    cue in this file.
                  </div>
                )}
