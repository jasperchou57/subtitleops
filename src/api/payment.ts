...
export const createCheckoutSession = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (existingSubscription) {
      throw new Error(
        'An existing subscription must be managed from the billing page.'
      );
    }
  ...
  .handler(async ({ context }) => {
    ...
    for (const rec of payments) {
      if (
        ...
        const plan =
          plans.find((candidate) => candidate.id === rec.planId) ??
          findPlanByPriceId(rec.priceId);
    ...
    if (activeSubscription) {
      const subscriptionPlan =
        plans.find((candidate) => candidate.id === activeSubscription.planId) ??
        plans.find((p) =>
