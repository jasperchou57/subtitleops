    ...
    <div className="rounded-xl border bg-card p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="text-sm">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">Seconds before the first cue.</span>
          <input
            type="number"
            min="0"
            max="86400"
            step="0.5"
            value={startSeconds}
            onChange={(event) => setStartSeconds(clampNumber(Number(event.target.value), 0, 86400))}
        ...
        <label className="text-sm">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">How long each subtitle stays visible.</span>
          <input
            type="number"
            min="0.5"
            max="30"
            ...
            onChange={(event) => setSecondsPerLine(clampNumber(Number(event.target.value), 0.5, 30))}
            onBlur={() => trackSetting("seconds_per_line", String(secondsPerLine))}
            className="mt-2 h-9 w-full rounded-lg border bg-background px-3 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
          />
        </label>
        ...
        <label className="text-sm">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">Pause between subtitle cues.</span>
          <input
            type="number"
            min="0"
            max="10"
            step="0.1"
            value={gap}
            onChange={(event) => setGap(clampNumber(Number(event.target.value), 0, 10))}
        ...
        <label className="text-sm">
          <span className="font-medium text-foreground">Max chars per line</span>
          <span className="mt-1 block text-xs text-muted-foreground">Wrap long cues for readability.</span>
          <input
            type="number"
            min="12"
            max="80"
            ...
            onChange={(event) => setMaxCharsPerLine(clampNumber(Number(event.target.value), 12, 80))}
            onBlur={() => trackSetting("max_chars_per_line", String(maxCharsPerLine))}
            className="mt-2 h-9 w-full rounded-lg border bg-background px-3 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
          />
        </label>
      </div>
  ...
  return (
    <GenericConverter
      ...
      directInput={{
        ...
        helperText: "Use the timing settings above before converting pasted text.",
        placeholder: "Welcome to the video.\nToday we will cover the basics.\nLet's start with the first step.",
