      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          if (fileRecord && !fileRecord.isPublic) {
            let hasAccess = Boolean(userId && fileRecord.userId === userId);
            if (!hasAccess && userId) {
              const [sharedAccess] = await db
                .select({ id: conversionProjects.id })
                .from(conversionProjects)
                .innerJoin(
                  workspaceMembers,
                  eq(
                    workspaceMembers.workspaceId,
                    conversionProjects.workspaceId
                  )
                )
                .where(
                  and(
                    eq(conversionProjects.archiveFileId, fileRecord.id),
