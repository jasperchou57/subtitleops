...
import { BetaSignupButton } from './beta-signup-button';
...
export function PricingCard({
  ...
  const isAuthenticated = !!metadata?.userId;
  ...
  return (
    <Card
      className={cn(
        ...
        plan.popular && 'relative overflow-visible ring-2 ring-primary',
        className
      )}
    >
      {plan.popular && (
      ...
      <CardHeader>
        ...
        {interval === PlanIntervals.YEAR && price && (
          <p className="-mt-3 mb-4 text-xs text-muted-foreground">
            {formatPrice(Math.round(price.amount / 12), price.currency)}/month,
            billed annually
          </p>
        )}
        <CardDescription>
        ...
        {plan.isFree ? (
          ...
          <BetaSignupButton
            planId={plan.id === 'studio' ? 'studio' : 'pro'}
          ...
          isAuthenticated && hasValidPriceId ? (
            ...
            >
              {plan.isLifetime
                ...
                : plan.id === 'studio'
                  ? m.pricing_card_get_started_studio()
