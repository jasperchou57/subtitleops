...
export function ConversionResult({
  ...
  return (
    <div
      data-analytics-area="conversion_result"
      className="mt-6 rounded-xl border bg-card p-6 animate-in fade-in slide-in-from-bottom-4 duration-500"
    >
      ...
      <div className="mb-5 rounded-lg bg-muted/30 px-4 py-3 text-sm space-y-1">
        <p className="text-foreground">
          ...
          {stats.duration && (
            <span className="text-muted-foreground">
              {' '}
              &middot; {stats.duration} total duration
            </span>
          )}
        </p>
        {stats.longLineCount > 0 && (
          <p className="text-amber-600">
            ...
            {stats.longLineCount}{' '}
            {stats.longLineCount === 1 ? 'line exceeds' : 'lines exceed'} 42
            characters — may overflow on small screens
          </p>
        )}
      </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Your file is processed locally and won&apos;t be available after you
        leave this page.
      </p>
