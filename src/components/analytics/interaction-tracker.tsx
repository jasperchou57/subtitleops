...
function getArea(element: HTMLElement): string {
  ...
  if (section) {
    ...
    if (heading?.textContent)
      return normalizeLabel(heading.textContent, 'section');
  }
...
function getElementType(element: HTMLElement): string {
  if (element instanceof HTMLInputElement)
    return `input_${element.type || 'text'}`;
...
function getTool(element: HTMLElement): string | undefined {
  const explicitTool = element.closest<HTMLElement>('[data-analytics-tool]')
    ?.dataset.analyticsTool;
...
export function InteractionTracker() {
  useEffect(() => {
    ...
    const handleChange = (event: Event) => {
      ...
      if (shouldTrackChange) sendInteraction(element as HTMLElement, 'change');
    };
