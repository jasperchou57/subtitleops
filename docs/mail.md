...
Transactional email (verification, password reset, workspace invitation, contact form, subscription welcome). **Resend** and **Cloudflare Email Service** are the built-in providers. Design allows adding other providers via a provider registry without changing callers.
...
| workspaceInvitation | `{ url, workspaceName, role }` | Join a SubtitleOps workspace |
