...
| `VITE_MAIL_ENABLED` | Enable verification and password-reset email UI | No | Set to `true` only when the mail provider is configured |
| **Payment** | | | |
...
| `VITE_STRIPE_PRICE_STUDIO_MONTHLY` | Stripe Price ID (Studio monthly) | No | |
| `VITE_STRIPE_PRICE_STUDIO_YEARLY` | Stripe Price ID (Studio yearly) | No | |
| **Payment (Creem)** | | | |
...
| `VITE_CREEM_PRODUCT_STUDIO_MONTHLY` | Creem Product ID (Studio monthly) | No | |
...
| `GOOGLE_CLIENT_ID` | Google OAuth client ID | Yes (prod) | Callback: `/api/auth/callback/google` |
...
`OAUTH_GITHUB_CLIENT_SECRET`; the deploy workflow maps them to the runtime
