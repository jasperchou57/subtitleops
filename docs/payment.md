...
- **Payment callback**: `/settings/payment?session_id=...&callback=/settings/billing` — polls until paid, then redirects.
...
1. **Env**: Set the following environment variables (see [Env](./env.md)):
   ...
   - **Build-time:** `VITE_PAYMENT_PROVIDER=stripe`, plus monthly and annual Stripe Price IDs for Pro and Studio.
...
1. **Env**: Set the following environment variables (see [Env](./env.md)):
   ...
   - **Build-time:** `VITE_PAYMENT_PROVIDER=creem`, plus monthly and annual Creem Product IDs for Pro and Studio.
...
3. **Creem Dashboard**:
   - Create Products for Pro and Studio (monthly/yearly).
